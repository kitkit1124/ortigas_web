...
class BF_Model extends CI_Model
{
	...
	public function insert($data = null)
	{
		...
		if ($status == false) {
			...
			$this->_audittrail('insert', $this->db->last_query()); // log this
	...
	public function update($where = null, $data = null)
	{
		...
		log_message('debug', print_r($where, TRUE));
		...
		if ($result = $this->db->update($this->table_name, $data, $where)) {
			$this->_audittrail('update', $this->db->last_query()); // log this
	...
	public function delete($id = null)
	{
		...
		if ($result) {
			$this->_audittrail('delete', $this->db->last_query()); // log this
	...
	private function _audittrail($action, $sql)
	{
		// TODO: improve this

		// log these events
		// $events = array('before_insert', 'after_update', 'before_delete');
		...
		// {
			...
			$data = array(
				...
				'audittrail_table'				=> $this->table_name,
				'audittrail_data'				=> json_encode($sql),
				'audittrail_user_ip'			=> $this->input->ip_address(),
				'audittrail_user_agent'			=> $this->input->user_agent(),
				'audittrail_created_by' 		=> $this->session->userdata('user_id'),
				'audittrail_created_on' 		=> date('Y-m-d H:i:s'),
			);

			$this->db->insert('audittrails', $data);
