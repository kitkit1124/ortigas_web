		...
		<?php foreach ($room_types as $key => $value) { ?>
			<li class="nav-item">
				<a class="" data-toggle="tab" data-id="<?php echo $value->room_type_id; ?>" href="#room-menu<?php echo $key; ?>">
					<img class="unit_floorplan_thumb lazy" data-src="<?php echo getenv('UPLOAD_ROOT').$value->room_type_image; ?>" alt="<?php echo $value->room_type_alt_image; ?>" title="<?php echo $value->room_type_alt_image; ?>" data-title="<?php echo $value->room_type_name; ?>" />	
				</a>
			</li>
    	<?php } ?>
    </ul>
	...
	<div class="tab-content">
		...
		<div id="room-menu<?php echo $key; ?>" class="tab-pane fade">
			<a href="<?php echo site_url().'properties/properties/floorplan_image?img='.getenv("UPLOAD_ROOT").$value->room_type_image ?>" data-target="#modal-lg" data-toggle="modal">
				<img class="lazy" data-src="<?php echo getenv('UPLOAD_ROOT').$value->room_type_image; ?>" alt="<?php echo $value->room_type_alt_image; ?>" title="<?php echo $value->room_type_alt_image; ?>" />
			</a>
    	</div>
